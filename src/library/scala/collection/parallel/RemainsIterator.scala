...
self =>
  ...
  class RITaken(tk: Int) extends super.Taken(tk) with SeqSplitter[T] {
  ...
  override private[collection] def newTaken(until: Int): RITaken = new RITaken(until)
  ...
  class RIMapped[S](f: T => S) extends super.Mapped[S](f) with SeqSplitter[S] {
  ...
  def appendParSeq[U >: T, PI <: SeqSplitter[U]](that: PI) = new RIAppended[U, PI](that)

  class RIZipped[S](ti: SeqSplitter[S]) extends super.Zipped[S](ti) with SeqSplitter[(T, S)] {
  ...
  override def zipAllParSeq[S, U >: T, R >: S](that: SeqSplitter[S], thisElem: U, thatElem: R) = new RIZippedAll[U, R](that, thisElem, thatElem)
