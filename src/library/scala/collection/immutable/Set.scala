...
/** Base trait for immutable set operations
  ...
  * @define Coll `immutable.Set`
  */
trait SetOps[+A, +CC[+X] <: SetOps[X, CC, _], +C <: SetOps[A, CC, C]]
  extends collection.SetOps[A, CC, C] {

  protected def coll: C with CC[A]
  ...
  /** Creates a new set with an additional element, unless the element is
    ...
    *          contains `elem`.
    */
  def incl[A1 >: A](elem: A1): CC[A1]

  /** Alias for `incl` */
  @deprecatedOverriding("This method should be final, but is not due to scala/bug#10853", "2.13.0")
  override /*final*/ def + [A1 >: A](elem: A1): CC[A1] = incl(elem) // like in collection.Set but not deprecated
  ...
  override def concat[A1 >: A](that: collection.Iterable[A1]): CC[A1] = {
    var result: CC[A1] = coll
...
object Set extends IterableFactory[Set] {
  ...
  private object EmptySet extends AbstractSet[Any] {
    ...
    def incl[A1 >: Any](elem: A1): Set[A1] = new Set1(elem)
    def excl[A1 >: Any](elem: A1): Set[Any] = this
  ...
  final class Set1[+A] private[collection] (elem1: A) extends AbstractSet[A] with StrictOptimizedIterableOps[A, Set, Set[A]] {
    ...
    def incl[A1 >: A](elem: A1): Set[A1] =
      if (contains(elem)) this
      else new Set2(elem1, elem)
    def excl[A1 >: A](elem: A1): Set[A] =
